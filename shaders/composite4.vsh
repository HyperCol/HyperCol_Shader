...
void main() {
    ...
    sunColor = sky_transmittance(vec3(0.0, sky_planetRadius, 0.0), wSunVector, 8.) * 0.2;
	sunColor = normalize(sunColor + 0.001);
	sunColor *= pow(clamp01(wSunVector.y), 0.9);
	sunColor *= transitionFading;
    ...
    upperCloudSunlightColor = exp2(-(1.0 / (clamp01(wSunVector.y) + 0.001)) * vec3(0.3, 0.55, 1.0) * 0.3);
